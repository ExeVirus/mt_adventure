...
local function lavafall_timer(pos)
	...
	local id   = minetest.add_particlespawner({
		...
		vertical   = false,
		texture    = 'lava_white.png',
	})
	meta:set_int('spawner', id)
	return 1
end

local function lavafall_formspec(direction, height, distance, spread)
	local formspec =
		'size[5.5,5]'..
		default.gui_bg..
		default.gui_bg_img..
...
minetest.register_lbm({
	name 	  = 'falls:trigger_lavafalls',
	nodenames = {'falls:lavafall_block'},
...
minetest.register_node('falls:lavafall_block', {
	drawtype 	= 'glasslike',
	description = S('Lavafall Block'),
	tiles     	= {'lavafall_block.png'},
	alpha     	= 160,
	paramtype 	= 'light',
	walkable  	= false,
	drowning  	= 1,
	...
	on_construct = function(pos)
		...
		meta:set_string('formspec', lavafall_formspec(1,2,1,2))
	...
	on_receive_fields = function(pos, formname, fields, player)
		...
		if(fields.update) then
			...
			meta:set_string('formspec',
                lavafall_formspec((fields.direction + 45) / 45, ht, distance, spd))
		end
	end,
})

minetest.register_craft({
	output = 'falls:lavafall_block 1',
	recipe = {
...
minetest.register_node('falls:lavafall_block_inv', {
	drawtype 	= 'airlike',
	description = S('Invisible Lavafall Block'),
	inventory_image  = 'lavafall_block.png',
	paramtype 	= 'light',
	groups 		= {oddly_breakable_by_hand=3},
	sounds 		= default.node_sound_water_defaults(),
...
minetest.register_craft({
	output = 'falls:lavafall_block_inv 1',
	recipe = {
		{'default:glass'},
		{'falls:lavafall_block'},
	},
})
...
local function lava_basin_timer(pos)
	local meta = minetest.get_meta(pos)
	local id = minetest.add_particlespawner({
        amount = 400,
        time   = 10,
        ...
        collision_removal  = false,
		vertical = false,
		texture  = 'lava_blue.png',
    })
	meta:set_int('spawner', id)
	return 1
end

local function lava_basin_formspec(direction, height, distance, spread)
	local formspec =
		'size[5.5,5]'..
        default.gui_bg..
        default.gui_bg_img..
...
minetest.register_lbm({
	name      = 'falls:trigger_lava_basins',
	nodenames = {'falls:lava_basin'},
	run_at_every_load = true,
	action = function(pos, node)
		local meta = minetest.get_meta(pos)
        local id = minetest.add_particlespawner({
...
minetest.register_node('falls:lava_basin', {
	drawtype    = 'glasslike',
	description = S('Waterfall Basin'),
	tiles       = {'lavafall_basin.png'},
	walkable    = false,
	paramtype 	= 'light',
    climbable   = true,
	groups      = {oddly_breakable_by_hand=3},
	...
	on_construct = function(pos)
		...
		meta:set_string('formspec', lava_basin_formspec(1,2,1,2))
	...
	on_receive_fields = function(pos, formname, fields, player)
		...
		if(fields.update) then
			...
			minetest.get_node_timer(pos):start(10.0)
			meta:set_string('formspec',
                lava_basin_formspec((fields.direction + 45) / 45, ht, distance, spd))
		end
	end,
})

minetest.register_craft({
	output = 'falls:lava_basin 1',
	recipe = {
		{'','falls:lava_turbulent',''},
		{'default:sand','default:sand','default:sand'}
	},
	replacements = {{ 'falls:lava_turbulent', 'bucket:bucket_empty'}}
})

--
-- Lavafall Basin Invisible
--

minetest.register_node('falls:lava_basin_inv', {
	drawtype    = 'airlike',
	description = S('Invisible Lavafall Basin'),
	inventory_image  = 'lavafall_basin.png',
	groups      = {oddly_breakable_by_hand=3},
	sounds      = default.node_sound_water_defaults(),
...
local function lava_fountain_timer(pos)
	local id = minetest.add_particlespawner({
        amount  = 400,
        time    = 10,
...
minetest.register_lbm({
	name      = 'falls:trigger_lava_fountains',
	nodenames = {'falls:lava_fountain'},
	run_at_every_load = true,
	action = function(pos, node)
        local meta = minetest.get_meta(pos)
        local id = minetest.add_particlespawner({
...
minetest.register_node('falls:lava_fountain', {
	description     = S('Lava Fountain'),
	drawtype        = 'plantlike',
	tiles           = {'lava_fountain.png'},
	inventory_image = 'lava_fountain_inventory.png',
	groups          = {oddly_breakable_by_hand=3},
	sounds          = default.node_sound_stone_defaults(),
    use_texture_alpha = true,
	paramtype 	= 'light',

    on_timer = lava_fountain_timer,
